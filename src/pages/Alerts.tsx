...
import { AlertTriangle, CheckCircle, Bell, MapPin, Clock } from "lucide-react";
import { alerts as initialAlerts, AlertSeverity } from "@/data/mock";
import { cn } from "@/lib/utils";

type Filter = "all" | "critical" | "active";

const severityConfig: Record<AlertSeverity, { border: string; bg: string; text: string; icon: any; label: string }> = {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {initialAlerts.filter((a) => a.severity === "critical").length} critical · {initialAlerts.length - resolved.size} active
        </p>
      </div>
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="mt-2 flex flex-wrap gap-4 text-xs text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {a.timeAgo}</span>
                  </div>
                </div>
                <button
